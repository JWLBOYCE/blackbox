...
struct FlightsView: View {
    ...
    var body: some View {
        FlightWorkspaceView(store: store)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 92), spacing: 8)], spacing: 8) {
                ...
                TotalsChip("PICUS Day", LogbookFormatters.hours(totals.picusDayMinutes))
                TotalsChip("PICUS Night", LogbookFormatters.hours(totals.picusNightMinutes))
            ...
            if !compact {
                VStack(alignment: .leading, spacing: 2) {
                    Text(flight.routeDisplay.isEmpty ? "No route" : flight.routeDisplay.replacingOccurrences(of: " -> ", with: "->"))
                        .lineLimit(1)
                    Text("\(LogbookFormatters.zuluTimeFormatter.string(from: flight.date))->\(LogbookFormatters.zuluTimeFormatter.string(from: flight.arrivalDate))")
...
private struct ArrivalZuluField: View {
    ...
    var body: some View {
        HStack {
            Text("Arrival (Zulu)")
            Spacer()
            Text(LogbookFormatters.zuluTimeFormatter.string(from: flight.arrivalDate))
                .font(.callout.monospacedDigit())
                .foregroundStyle(OpenPilotTheme.cyan)
        }
        .fieldShell()
        .accessibilityElement(children: .combine)
        .accessibilityLabel("Arrival Zulu")
        .accessibilityValue(LogbookFormatters.zuluTimeFormatter.string(from: flight.arrivalDate))
    }
}
                                        ...
                                        Grid(alignment: .leading, horizontalSpacing: 18, verticalSpacing: 12) {
                                            GridRow {
                                                LabeledContent("Entry") {
                                                    Picker("", selection: binding.entryKind) {
                                ...
                                Grid(alignment: .leading, horizontalSpacing: 18, verticalSpacing: 12) {
                                    ...
                                    FlightSection("Times and Landings", systemImage: "clock") {
                                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 220), spacing: 12)], spacing: 12) {
                                            ...
                                            MinutesStepper("PICUS Day", minutes: binding.picusDayMinutes)
                                            MinutesStepper("PICUS Night", minutes: binding.picusNightMinutes)
...
private struct CrewNamesField: View {
    ...
    private var otherCrewText: String {
        let assigned = Set(
            FlightEntry.splitCrewNames(captainName)
                + FlightEntry.splitCrewNames(firstOfficerName)
        )
        return names
    ...
    private func binding(for slot: CrewSlot) -> Binding<String> {
        Binding {
            ...
            let movedNames = Set(FlightEntry.splitCrewNames(normalized))
            let captain = FlightEntry.splitCrewNames(captainName)
            let firstOfficer = FlightEntry.splitCrewNames(firstOfficerName)
            let other = FlightEntry.splitCrewNames(otherCrewText)
            switch slot {
            case .captain:
                updateCrew(
                    captain: normalized,
                    firstOfficer: firstOfficer.filter { !movedNames.contains($0) }.joined(separator: " | "),
                    other: other.filter { !movedNames.contains($0) }.joined(separator: " | ")
                )
            case .firstOfficer:
                updateCrew(
                    captain: captain.filter { !movedNames.contains($0) }.joined(separator: " | "),
                    firstOfficer: normalized,
                    other: other.filter { !movedNames.contains($0) }.joined(separator: " | ")
                )
            case .other:
                updateCrew(
