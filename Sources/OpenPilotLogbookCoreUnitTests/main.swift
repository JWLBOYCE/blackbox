...
func testRecencyAndDuplicates() {
    ...
    let recency = LogbookAnalysis.recencySnapshot(flights: [recent, duplicate, old], now: now)
    expect(recency.hoursLast12Months == 120, "last-12-month hours should include only recent entries")
    ...
    expect(recency.instrumentLast90Days == 120, "90-day instrument time should total recent entries")
