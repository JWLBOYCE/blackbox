...
public enum SolarDayNightCalculator {
    ...
    public static func nightMinutes(
        ...
        twilightElevation: Double = civilTwilightElevation
    ) -> Int {
        ...
        for minute in stride(from: 0, to: durationMinutes, by: step) {
            ...
            if solarElevation(date: date, latitude: position.latitude, longitude: position.longitude) < twilightElevation {
                night += segment
            }
        }
        return roundToNearestFiveMinutes(night, maximum: durationMinutes)
    }

    public static func roundToNearestFiveMinutes(_ minutes: Int, maximum: Int? = nil) -> Int {
        let rounded = max(0, Int((Double(minutes) / 5).rounded()) * 5)
        return maximum.map { min(rounded, max(0, $0)) } ?? rounded
    }

    public static func isNight(date: Date, latitude: Double, longitude: Double) -> Bool {
        solarElevation(date: date, latitude: latitude, longitude: longitude) < civilTwilightElevation
    }
