...
public final class LogbookRepository {
    ...
    public func bootstrapIfNeeded() throws {
        ...
        if try setting("enriched_import_v6", in: db) != "true" {
            try enrichFromLogTenBackup(into: db)
    ...
    public func save(_ flight: FlightEntry) throws -> Int64 {
        ...
        if let id = flight.id {
            ...
            UPDATE flights SET
              ...
              total_minutes = ?, pic_minutes = ?, pic_day_minutes = ?, pic_night_minutes = ?, picus_minutes = ?,
    ...
    public func personSummaries() throws -> [PersonSummary] {
        let db = try SQLiteConnection(path: paths.workingDatabase.path)
        let rows = try db.rows("SELECT crew_names, total_minutes FROM flights WHERE crew_names != ''")
        ...
        if flight.entryKind == "Flight" {
            ...
            if flight.pilotFlying {
                ...
                if flight.pilotFunction.isEmpty || flight.pilotFunction == "Co-pilot" {
                    flight.pilotFunction = "PIC"
                }
            } else if flight.pilotFunction == "PIC" || flight.pilotFunction == "PICUS" {
                flight.picMinutes = flight.totalMinutes
                flight.picNightMinutes = min(flight.totalMinutes, flight.nightMinutes)
                flight.picDayMinutes = max(0, flight.totalMinutes - flight.picNightMinutes)
                if flight.pilotFunction == "PICUS" { flight.picusMinutes = flight.totalMinutes }
                flight.copilotMinutes = 0
                flight.copilotDayMinutes = 0
                flight.copilotNightMinutes = 0
            } else {
    ...
    private func backfillSectorNotes(in db: SQLiteConnection) throws {
        ...
        WHERE remarks = 'Imported from BA roster PDF'
    ...
    private static func prependingSectorNote(to remarks: String, nextSector: Int) throws -> String {
        ...
        guard !trimmed.isEmpty, trimmed != "Imported from BA roster PDF" else { return "Sector \(nextSector)" }
    ...
    private static func normalizedCrewRoles(_ text: String, names: [String], pilotFunction: String) -> String {
        ...
        let ownName = "James Boyce"
        if names.contains(ownName), !pilotFunction.isEmpty {
            roles[ownName] = pilotFunction == "Co-pilot" ? "First Officer" : (pilotFunction == "PIC" ? "Captain" : pilotFunction)
        } else if let first = names.first, roles[first] == nil {
            roles[first] = pilotFunction.isEmpty ? "Pilot" : (pilotFunction == "PIC" ? "Captain" : pilotFunction)
        }
        if names.contains(ownName), pilotFunction == "Co-pilot",
           !names.contains(where: { roles[$0] == "Captain" || roles[$0] == "PIC" }),
           let captain = names.first(where: { $0 != ownName }) {
    ...
    private func logTenFlightRows(from source: SQLiteConnection) throws -> [[String: SQLiteValue]] {
        ...
        SELECT f.Z_PK AS source_pk,
               ...
               COALESCE(f.ZFLIGHT_MULTIPILOT, 0) AS multipilot,
                       ...
                       COALESCE(f.ZFLIGHT_SIC, 0) + COALESCE(f.ZFLIGHT_P1US, 0) AS copilot_minutes,
               MAX(
                   COALESCE(f.ZFLIGHT_SIC, 0) + COALESCE(f.ZFLIGHT_P1US, 0) -
                   MIN(
                       COALESCE(f.ZFLIGHT_SIC, 0) + COALESCE(f.ZFLIGHT_P1US, 0),
                       CASE
                           WHEN COALESCE(f.ZFLIGHT_SICNIGHT, 0) + COALESCE(f.ZFLIGHT_P1USNIGHT, 0) > 0
                           THEN COALESCE(f.ZFLIGHT_SICNIGHT, 0) + COALESCE(f.ZFLIGHT_P1USNIGHT, 0)
                           ELSE COALESCE(f.ZFLIGHT_NIGHT, 0)
                       END
                   ),
                   0
               ) AS copilot_day_minutes,
               MIN(
                   COALESCE(f.ZFLIGHT_SIC, 0) + COALESCE(f.ZFLIGHT_P1US, 0),
                   CASE
                       WHEN COALESCE(f.ZFLIGHT_SICNIGHT, 0) + COALESCE(f.ZFLIGHT_P1USNIGHT, 0) > 0
               ...
               COALESCE(f.ZFLIGHT_SFI, 0) + COALESCE(f.ZFLIGHT_DUALGIVEN, 0) AS instructor_minutes,
                       ...
                       COALESCE(f.ZFLIGHT_TOTALINSTRUMENT, 0) AS instrument_minutes,
    ...
    private static func flightFromLogTen(row: [String: SQLiteValue]) -> FlightEntry {
        ...
        if copilot == 0, pic == 0, dual == 0, instructor == 0, fstd == 0, total > 0 {
            if isMultiPilot {
                copilot = total
            } else {
                pic = total
        ...
        return FlightEntry(
            ...
            date: Date(timeIntervalSinceReferenceDate: row["flight_date"]?.double ?? 0),
            departure: departure,
            arrival: arrival,
